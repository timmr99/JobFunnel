...
class Indeed(JobFunnel):
    ...
    def search_page_for_job_soups(self, search, page, job_soup_list):
        ...
        jobs = BeautifulSoup(
            self.session.get(url, headers=self.headers).text, self.bs4_parser). \
    ...
    def search_joblink_for_blurb(self, job):
        ...
        job_link_soup = BeautifulSoup(
            self.session.get(search, headers=self.headers).text, self.bs4_parser)
    ...
    def get_blurb_with_delay(self, job, delay):
        ...
        res = self.session.get(search, headers=self.headers).text
        return job, res
    ...
    def scrape(self):
        ...
        request_html = self.session.get(search, headers=self.headers)
